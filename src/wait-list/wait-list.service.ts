...
export class WaitListService {
    ...
    async addToWaitlist(userId: number, groupeId: number) {
        ...
        //  empêcher de s'inscrire sur la waitlist d'un groupe déjà passé
        const maintenant = new Date();
        if (new Date(groupe.dateFin) < maintenant) {
            throw new BadRequestException('Ce groupe est déjà terminé, vous ne pouvez pas rejoindre la liste d\'attente');
        }
